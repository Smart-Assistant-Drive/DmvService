...
class LicenceController(licenceInput: LicenceInputBoundary) {
    ...
    @GetMapping("/licence")
    @Operation(summary = "Get licence",
        description = "Get licence",
        parameters = [Parameter(name = "licenceId", description = "Licence id to be retrieved")],
        responses = [
    ...
    fun getLicence(@RequestParam(value = "id", defaultValue = "") licenceId: String?): HttpEntity<LicenceResponseDto> {
        val links = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(LicenceController::class.java).getLicence(licenceId))
            .withSelfRel()
    ...
    @Operation(summary = "Create licence",
        ...
        requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
            ...
            required = true),
        responses = [
            ApiResponse(responseCode = "201", description = "Created licence",
                content = [Content(mediaType = "application/json",
                    schema = Schema(implementation = LicenceResponseDto::class)
                )]),
            ApiResponse(responseCode = "400", description = "Invalid licence",
                content = [Content()]),
            ApiResponse(responseCode = "409", description = "Valid licence already exists",
                content = [Content()]),
            ApiResponse(responseCode = "500", description = "Internal server error",
                content = [Content()])])
    fun newLicence(@RequestBody licence: LicenceRequestModel): HttpEntity<LicenceResponseDto> {
        val links = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(LicenceController::class.java).newLicence(licence))
            .withSelfRel()

        val result = input.addLicence(licence.name, licence.surname, licence.birthDate, licence.licenceCountry, licence.residence)
        return if (result.isSuccess) {
    ...
    @Operation(summary = "Update vehicle",
        ...
        parameters = [Parameter(name = "vin", description = "Licence id to be updated", `in` = ParameterIn.PATH)],
        requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
            content = [Content(mediaType = "application/json",
        ...
        responses = [
            ...
            ApiResponse(responseCode = "400", description = "Invalid licence information",
                content = [Content()]),
            ApiResponse(responseCode = "404", description = "Licence not found",
                content = [Content()]),
            ApiResponse(responseCode = "500", description = "Internal server error",
                content = [Content()])])
    fun updateLicence(@RequestBody licence: LicenceUpdateModel, @PathVariable id: String): HttpEntity<LicenceResponseDto> {
        val links = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(LicenceController::class.java).updateLicence(licence, id))
            .withSelfRel()
